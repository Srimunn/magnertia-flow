...
function ExpensesPage() {
  ...
  const columns: Column<Expense>[] = [
    { key: "id", header: "Reference", cell: (r) => <span className="font-mono text-xs font-semibold text-foreground">{r.id}</span> },
    { key: "date", header: "Date", cell: (r) => <span className="text-muted-foreground">{r.date}</span> },
    { key: "category", header: "Category", cell: (r) => (
      <span className="inline-flex items-center rounded-md bg-secondary px-2 py-0.5 text-xs font-medium text-secondary-foreground">{r.category}</span>
    )},
    ...
    { key: "actions", header: "", align: "right", cell: () => (
      <button className="rounded-md p-1.5 text-muted-foreground hover:bg-secondary hover:text-foreground"><MoreHorizontal className="h-4 w-4" /></button>
    )},
  ];
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={
          <>
            <ErpButton variant="outline" size="md"><Filter className="h-4 w-4" /><span className="hidden sm:inline">Filter</span></ErpButton>
            <ErpButton variant="outline" size="md"><Download className="h-4 w-4" /><span className="hidden sm:inline">Export</span></ErpButton>
            <ErpButton size="md"><Plus className="h-4 w-4" /><span className="hidden sm:inline">Add Expense</span></ErpButton>
          </>
        }
      />

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Total Expenses (YTD)" value={formatCurrency(total, true)} icon={Wallet} tone="warning"
          delta={{ value: "3.2%", positive: false }} />
        <KpiCard label="Electricity Cost" value={formatCurrency(expenseCategories[0].amount, true)} icon={Wallet} hint="43% of total" />
        <KpiCard label="Pending Approvals" value="3" icon={Wallet} tone="warning" hint="₹277K pending" />
        <KpiCard label="Recurring Monthly" value={formatCurrency(3200000, true)} icon={Wallet} hint="Auto-scheduled" />
      </div>

      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <div className="card-soft p-5">
          <h3 className="font-display text-base font-semibold text-foreground">Expense Breakdown</h3>
          <p className="text-xs text-muted-foreground">By category · YTD</p>
          <div className="mt-3 h-[260px]">
            <ResponsiveContainer>
              <PieChart>
                <Pie data={expenseCategories} dataKey="amount" nameKey="category" innerRadius={50} outerRadius={90} paddingAngle={2}>
        ...
        <div className="card-soft p-5 lg:col-span-2">
          <h3 className="font-display text-base font-semibold text-foreground">Category Performance</h3>
              ...
              return (
                <li key={c.category}>
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-semibold tabular-nums text-foreground">{formatCurrency(c.amount, true)}<span className="ml-2 text-xs text-muted-foreground">{pct.toFixed(1)}%</span></span>
                  </div>
            ...
            <>
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 text-xs text-muted-foreground">{r.category} · {r.date}</div>
