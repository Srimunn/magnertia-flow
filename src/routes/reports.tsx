...
function ReportsPage() {
  ...
  const cashFlow = revenueTrend.map(r => ({ month: r.month, inflow: r.revenue, outflow: -r.expenses }));
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={<ErpButton variant="outline" size="md"><Download className="h-4 w-4" /><span className="hidden sm:inline">Export PDF</span></ErpButton>}
      />
        ...
        {RANGES.map((r) => (
          <button
            ...
            className={cn(
              ...
              range === r ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-secondary hover:text-foreground",
            )}
          >
            {r}
          </button>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <ChartCard title="Revenue Trend" subtitle={`Period · ${range}`}>
          <AreaChart data={revenueTrend}>
            ...
            <YAxis stroke="var(--muted-foreground)" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `${(v/100000).toFixed(0)}L`} />
        ...
        <ChartCard title="Expense Trend" subtitle={`Period · ${range}`}>
          <AreaChart data={revenueTrend}>
            ...
            <YAxis stroke="var(--muted-foreground)" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `${(v/100000).toFixed(0)}L`} />
            <Tooltip contentStyle={tooltip} formatter={(v: number) => formatCurrency(v, true)} />
            <Area type="monotone" dataKey="expenses" stroke="var(--chart-5)" strokeWidth={2.5} fill="url(#r2)" />
          </AreaChart>
        </ChartCard>
        ...
        <ChartCard title="Profit Trend" subtitle="Revenue minus expenses">
          <LineChart data={profitTrend}>
            ...
            <YAxis stroke="var(--muted-foreground)" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `${(v/100000).toFixed(0)}L`} />
            <Tooltip contentStyle={tooltip} formatter={(v: number) => formatCurrency(v, true)} />
            <Line type="monotone" dataKey="profit" stroke="var(--chart-3)" strokeWidth={2.5} dot={{ r: 3 }} />
          </LineChart>
        </ChartCard>
        ...
        <ChartCard title="Cash Flow" subtitle="Inflow vs outflow">
          <BarChart data={cashFlow} stackOffset="sign">
            <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
            <XAxis dataKey="month" stroke="var(--muted-foreground)" fontSize={11} tickLine={false} axisLine={false} />
            <YAxis stroke="var(--muted-foreground)" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `${(Math.abs(v)/100000).toFixed(0)}L`} />
            <Tooltip contentStyle={tooltip} formatter={(v: number) => formatCurrency(Math.abs(v), true)} />
      ...
      <div className="mt-6 card-soft p-5">
        <h3 className="font-display text-base font-semibold text-foreground">Top Revenue Stations</h3>
        ...
        <div className="mt-4 h-[300px]">
          <ResponsiveContainer>
            <BarChart data={revenueByStation} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" vertical={false} />
              <XAxis dataKey="station" stroke="var(--muted-foreground)" fontSize={10} tickLine={false} axisLine={false}
                angle={-20} textAnchor="end" height={70} interval={0} />
              <YAxis stroke="var(--muted-foreground)" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(v) => `${(v/100000).toFixed(0)}L`} />
              <Tooltip contentStyle={tooltip} formatter={(v: number) => formatCurrency(v, true)} cursor={{ fill: "var(--secondary)" }} />
...
const tooltip = { background: "var(--card)", border: "1px solid var(--border)", borderRadius: 12, fontSize: 12 } as const;
