...
export function AutomationDevelopmentListPage({
  ...
  const { data: records = [], isLoading, refetch } = useQuery<AutomationDevelopment[]>({
    queryKey: ["automationDevelopmentRecords"],
    queryFn: () => automationDevelopmentService.listRecords(),
  });

  const filteredRecords = records.filter((rec) => {
    const matchesSearch =
      rec.id.toLowerCase().includes(searchTerm.toLowerCase()) ||
      rec.automationProjectTitle.toLowerCase().includes(searchTerm.toLowerCase()) ||
      rec.manufacturingPlant.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-card p-4 rounded-xl border border-border shadow-sm">
          <div>
            ...
            <h1 className="text-2xl font-bold text-foreground tracking-tight">Automation Development Directory</h1>
            <p className="text-xs text-muted-foreground">
              Automated production cell project directory tracking PLC, HMI, SCADA, robot programming, and deployment release.
            </p>
          </div>

          <Link
            to="/manufacturing-development/automation-development/new"
          ...
          >
            <Plus className="w-4 h-4" /> New Automation Project
          </Link>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-card p-3 rounded-lg border border-border">
          <div className="relative w-full sm:w-80">
            ...
            <input
              type="text"
              placeholder="Search ID, title, plant, product..."
              value={searchTerm}
              ...
              <table className="w-full text-xs text-left">
                <thead className="bg-muted/80 text-muted-foreground uppercase font-bold text-[10px] tracking-wider border-b border-border">
                  <tr>
                    ...
                    <th className="px-4 py-3">Automation Readiness</th>
                  ...
                  {filteredRecords.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="px-4 py-8 text-center text-muted-foreground">
                        No Automation Development projects found.
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map((rec) => {
                      const overall = calculateOverallAutomationReadiness({
                        productivityScore: rec.productivityScore,
                        qualityImprovementScore: rec.qualityImprovementScore,
                        costSavingScore: rec.costSavingScore,
                        energyEfficiencyScore: rec.energyEfficiencyScore,
                        aiHealthScore: rec.aiAutomationHealthScore,
                      });
                      ...
                      return (
                        <tr key={rec.id} className="hover:bg-muted/40 transition-colors">
                          ...
                          <td className="px-4 py-3 font-medium text-foreground">
                            {rec.manufacturingPlant} — {rec.productionLine}
                          </td>
                          ...
                          <td className="px-4 py-3 font-extrabold text-emerald-600">
                            {formatINR(rec.roiEstimateInr, { style: "crore" })}
                          </td>
