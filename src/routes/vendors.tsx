...
import { vendors, formatCurrency } from "@/lib/mock-data";
...
function VendorsPage() {
  ...
  const columns: Column<Row>[] = [
    ...
    { key: "name", header: "Vendor", cell: (r) => <span className="font-medium text-foreground">{r.name}</span> },
    ...
    { key: "out", header: "Outstanding", align: "right", cell: (r) =>
      r.outstanding > 0
        ? <span className="font-semibold text-destructive tabular-nums">{formatCurrency(r.outstanding, true)}</span>
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        actions={<ErpButton size="md"><Plus className="h-4 w-4" /><span className="hidden sm:inline">Add Vendor</span></ErpButton>}
      />
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <KpiCard label="Total Outstanding" value={formatCurrency(totalOutstanding, true)} icon={Building2} tone="warning" />
      </div>
          ...
          mobileCard={(r) => (
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">{r.category} · {r.id}</div>
              ...
              <div className="text-right">
                {r.outstanding > 0
                  ? <div className="font-semibold text-destructive tabular-nums">{formatCurrency(r.outstanding, true)}</div>
