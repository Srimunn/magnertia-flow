...
import type { ReviewerRow, ApprovalDecision } from "@/lib/pilot-production/types";
import { toast } from "sonner";

interface PilotProductionReviewTableProps {
  reviewers: ReviewerRow[];
  currentDecision: ApprovalDecision;
  comments: string;
  approvalDate?: string;
  onDecisionChange: (decision: ApprovalDecision, comments: string) => void;
  onToggleReviewerStatus?: (index: number) => void;
}

export const PilotProductionReviewTable: React.FC<PilotProductionReviewTableProps> = ({
  reviewers,
  currentDecision,
  comments,
  approvalDate,
  ...
  const [localDecision, setLocalDecision] = useState<ApprovalDecision>(currentDecision);
  const [localComments, setLocalComments] = useState<string>(comments || "");

  const allApproved = reviewers.every((r) => r.status === "Approved");

  const handleDecisionSubmit = (newDecision: ApprovalDecision) => {
    ...
    <div className="space-y-4">
      ...
      {!allApproved && (
        <div className="flex items-center gap-2 p-2.5 bg-amber-50 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 rounded border border-amber-200 text-xs">
          ...
          <span>
            Executive approval decision dropdown is locked until all 7 reviewer roles sign off with "Approved".
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
        <div>
          ...
          <select
            ...
            onChange={(e) => handleDecisionSubmit(e.target.value as ApprovalDecision)}
          ...
          >
            ...
            <option value="Additional Pilot Required">Additional Pilot Required</option>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Enter review comments or conditions..."
        ...
        <div>
          ...
          <input
            type="text"
            readOnly
            value={approvalDate || "Auto-set on approval"}
