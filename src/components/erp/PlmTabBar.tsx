...
export function PlmTabBar({
  ...
  activeTab?: PlmTabId;
  onTabChange?: (tabId: PlmTabId) => void;
  scores?: {
  ...
  const tabs = PLM_TABS.map((t) => {
    let b = t.badge;
    if (scores) {
      if (t.id === "configuration" && scores.configuration !== undefined) b = scores.configuration.toString();
      if (t.id === "engineering" && scores.engineering !== undefined) b = scores.engineering.toString();
      if (t.id === "manufacturing" && scores.manufacturing !== undefined) b = scores.manufacturing.toString();
      if (t.id === "service_support" && scores.service !== undefined) b = scores.service.toString();
      if (t.id === "change_management" && scores.risk !== undefined) b = scores.risk.toString();
