...
interface ApqpReviewApprovalPanelProps {
  record: ApqpRecord;
  onReviewDecision?: (decision: ApqpApprovalDecision, comments: string) => void;
}

export const ApqpReviewApprovalPanel: React.FC<ApqpReviewApprovalPanelProps> = ({
  record,
  onReviewDecision,
}) => {
    ...
    <div className="bg-card border border-border rounded-lg shadow-sm p-3 text-xs space-y-3">
      <div className="flex justify-between items-center pb-2 border-b border-border">
        <div className="flex items-center gap-1.5">
          ...
          <h2 className="font-bold text-foreground text-xs">Review & Approval Matrix</h2>
        </div>
      </div>
