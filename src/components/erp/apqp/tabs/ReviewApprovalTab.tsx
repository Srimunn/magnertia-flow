    ...
    <div className="space-y-6 text-xs">
      <div className="bg-card p-4 rounded-lg border border-border flex justify-between items-center">
        <div>
          <h2 className="text-base font-bold text-foreground">
            APQP Executive Gate Review Board (8-Role Sign-off)
          </h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            Cross-functional approval matrix for APQP Manager, Design, Manufacturing, Quality, Supplier Quality, Program Manager, Plant Head, and COO sign-off.
          </p>
        </div>
      ...
      <div className="bg-card p-4 rounded-lg border border-border space-y-4">
        <h3 className="font-bold text-foreground pb-2 border-b border-border flex items-center gap-2">
          <UserCheck className="w-4 h-4 text-primary" /> Record APQP Gate Decision
        </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setSelectedDecision(e.target.value as ApqpApprovalDecision)}
          ...
          <div>
            ...
            <textarea
              rows={3}
              placeholder="Enter review feedback, conditions, or gate requirements..."
              value={comments}
