...
export const RoboticsAttachmentsCard: React.FC<RoboticsAttachmentsCardProps> = ({
  attachments,
  onUploadAttachment,
}) => {
    ...
    <>
      <div className="bg-card text-card-foreground border border-border rounded-lg p-4 shadow-sm space-y-3">
                ...
                <div className="flex items-center gap-2 truncate">
                  ...
                  <div className="truncate">
                    ...
                    <span className="text-[10px] text-muted-foreground block">
                      {item.uploadedAt} • {item.fileSize || "3.2 MB"} • v{item.revision || 1}
                    </span>
                  </div>
                </div>
        ...
        >
          ...
          <span className="text-[10px] text-muted-foreground">Robot programs (.vpp, .mod), PDF up to 50MB</span>
        </div>
          ...
          <div className="bg-card text-card-foreground border border-border rounded-xl shadow-xl w-full max-w-2xl p-6 space-y-4 max-h-[85vh] flex flex-col">
            <div className="flex justify-between items-center border-b border-border pb-3">
              <h3 className="text-base font-bold text-foreground">
                All Enterprise Robotics Attachments ({attachments.length})
              </h3>
              <button
