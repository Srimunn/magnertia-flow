...
import type { ReviewerRow, RoboticsApprovalDecision } from "@/lib/robotics-integration/types";

interface RoboticsReviewTableProps {
  reviewers: ReviewerRow[];
  currentDecision: RoboticsApprovalDecision;
  comments: string;
  approvalDate?: string;
  onDecisionChange: (decision: RoboticsApprovalDecision, comments: string) => void;
  onToggleReviewerStatus?: (index: number) => void;
}

export const RoboticsReviewTable: React.FC<RoboticsReviewTableProps> = ({
  reviewers,
  currentDecision,
  comments,
  approvalDate,
  ...
  const [localDecision, setLocalDecision] = useState<RoboticsApprovalDecision>(currentDecision);
  const [localComments, setLocalComments] = useState<string>(comments || "");

  const allApproved = reviewers.every((r) => r.status === "Approved");

  const handleDecisionSubmit = (newDecision: RoboticsApprovalDecision) => {
    ...
    <div className="space-y-4">
      ...
      {!allApproved && (
        <div className="flex items-center gap-2 p-2.5 bg-amber-50 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 rounded border border-amber-200 text-xs">
          ...
          <span>
            Approval Decision dropdown is locked until all 9 roles sign off with "Approved".
          </span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
        <div>
          ...
          <select
            ...
            onChange={(e) => handleDecisionSubmit(e.target.value as RoboticsApprovalDecision)}
